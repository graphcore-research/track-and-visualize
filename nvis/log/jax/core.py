...
def is_weights(p: Tuple[jax.tree_util.DictKey]) -> bool:
    """
        Help fn to tell if a leaf node in an arbitrary depth pytree contains weights
    """
...
class JaxTracker(BaseTracker):
    ...
    def create_event_and_stash(self, value: Any, name: str, type: str, tensor_type: TT, stash=True, *args, **kwargs) -> None:
    ...
    def _stash_opt_state(self, opt_state: Tuple) -> None:
        """
            Creates a stash for each jax.Array containing optimizer state in the network. 
        ...
        for os in opt_state:
            ...
            if os.__class__.__orig_bases__[0] is NamedTuple and not isinstance(os,EmptyState):
    ...
    def _stash_gradients(self, grads: Dict) -> None:
        """
            Creates a stash for each jax.Array containing weight gradients in the network. 
    ...
    def step(self, 
        ...
        """
            ...
            Takes as optional arguments, the model_state, optimizer_state and gradients to capture stats on them.

            Args
                model_state (Dict | None): PyTree of the model parameters
                    Cannot be none if `model_state` arg was provided to the `track` method when initalisaing the JaxTracker
                optimizer_state (Tuple | None): Tuple containing the optax optimizer state object(s)
                    Cannot be none if `model_state` arg was provided to the `track` method when initalisaing the JaxTracker
                gradients (Dict | None): PyTree of the gradients w.r.t. the model parameters
        ...
        if self.track_gradients:
            assert not isinstance(gradients, NoneType), 'You must provide gradients to the step call, as JaxTracker initialised with `track_gradients == True`' 
            self._stash_gradients(
                grads=gradients
            )
        ...
        if self.track_weights:
            assert not isinstance(model_state, NoneType), 'You must provide model_state to the step call, as JaxTracker initialised with `model_state != None`, therefore it is configured to track stats on parameter tensors' 
            self._stash_model_weights(params=model_state)
        # capture opt_state
        if self.track_optimizer:
            assert not isinstance(optimizer_state, NoneType), 'You must provide `optimizer_state` to the step call, as JaxTracker initialised with `optimizer_state != None`, therefore it is configured to track stats on the optimizer state tensors' 
            self._stash_opt_state(opt_state=optimizer_state)


    def intercept(self,f: Callable,*args,**kwargs) -> Any:
        """
            Higher-order functions which facilitates the capturing of intermediate stats for
            the fwd and bwd passes. Simply executes the function and returns its args.
...
def track(
        ...
        init_step =None) -> JaxTracker:
    """
        Function for initialising the Jax (Flax) Tensor Tracker context manager.

        By default it tracks the stastics for  what we refer to as the Activations (i.e. the outputs of the forward method in the nn module). 
        However it can also track gradients, weights and optimiser state. 
        At the end of training it will write all the logs to a LogFrame (a `pd.DataFrame` which conforms to the schema outline in our docs) 
        ...
        Args:
            model_state (Dict | None): PyTree of the model parameters (output from `your_model.init()`)
            optimizer_state (Tuple | None): Tuple containing the optax optimizer state object(s) (output from `your_optimizer.init(model_state)`)
            ...
            use_wandb (bool): If you wish to push the Logframe(s) as artifacts and get summary numerics statistic in wandb. (`offload_inc` should be the same (or a multiple) of the increment being used to call `wandb.log')
