...
from ..log.jax import stash_hist,stash_scalar_stats, stash_full_tensor
from ..log.jax.stash_values import _concretize_as_list, _concretize_as_scalar, exp_histogram, _stash_scalar_stats
import jax
...
def test_stash_hist_cpu():
    t1,t0,tn = jnp.ones((B,N,M),dtype=jnp.float32), jnp.zeros((B,N,M),dtype=jnp.float32), gen_unit_gauss(B,N,M)
...
def test_stash_hist_cuda():
    ...
    assert np.sum(e1['hist']) == B*N*M and e1['bins'][np.argmax(e1['hist'])] == 1, f"{e1['bins'][np.argmax(e1['hist'])]}"

    assert np.sum(e0['hist']) == B*N*M and e0['bins'][np.argmax(e0['hist'])] == 0, f"{e0['bins'][np.argmax(e0['hist'])]}"
...
def test_stash_scalar_stats_cuda():
    ...
    s1,s0,sn = _concretize_as_scalar(fn(t1)),_concretize_as_scalar(fn(t0)),_concretize_as_scalar(fn(tn))
