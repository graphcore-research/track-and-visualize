...
class SnappingCrossHair:
    ...
    Args:
        ax (matplotlib.axes.Axes): This is the axis of the figure for which events are being listened to
        other_ax (List[matplotlib.axes.Axes]): This is the list of other axes 

    
    """
    def __init__(self, ax: matplotlib.axes.Axes, other_ax: List[matplotlib.axes.Axes], lines: Dict[str,matplotlib.lines.Line2D], sensitivity: int = 10): #type: ignore
        self.ax = ax
    ...
    def set_cross_hair_visible(self, visible):
        ...
        if not isinstance(self.other_ax,NoneType):
            others_need_redraw = any([vl.get_visible() != visible for vl in self.other_vlines])
    ...
    def on_mouse_move(self, event: matplotlib.backend_bases.MouseEvent): #type: ignore
        ctime = time.time()
        # print(ctime - self.last_draw_time, self.time_out)
        if not (event.inaxes is self.ax):
            ...
            def _format_text(ind: int) -> str:
                return '\n'.join([f'{k}={v["y"][ind]:1.2f}' for k,v in self.lines_data.items()])
