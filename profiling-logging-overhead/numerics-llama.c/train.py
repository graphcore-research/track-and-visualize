    ...
    for compile in [False]:
        for svf in stash_values:
            for which_to_track in all_perms:
                ...
                conf_name = f"{'-'.join(which_to_track)}-{svf.__name__}{'-compiled' if compile else ''}" if not baseline else f"baseline{'-compiled' if compile else ''}"
                logging.warning('-'*10 + conf_name.upper() + '-'*10)
                def trace_handler(p):
                    # init these up here, can override if init_from='resume' (i.e. from a checkpoint)
                    
                    with open(f'key_averages/{conf_name}.pkl', 'wb') as f:
                        pickle.dump(p.key_averages(),f)
                    p.export_chrome_trace(f"profiles/{conf_name}.json")
