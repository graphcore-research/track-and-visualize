...
def check_for_prefill(global_dict: Dict):
    ...
    # This would prefill zeros where needed \
    # (i.e. the first time the new bin edge is encountered)
    prefills = []
...
def global_stash_to_logframe(
    ...
    Args:
        global_stast (Dict[int,List[Dict]]): the dictionary \
            which contains all the lists of tensors statistic stashes.
    ...
    def _backfill(min_len: int):
        ...
        # add zeros to the counts for bin_edges \
        # which weren't included in the most recent stash added
        for global_key, global_value in df_dict.items():
            if "scalar_stats" in global_key:
                if len(global_value) < min_len:
                    df_dict[global_key].extend(
                        np.zeros(min_len - len(global_value)).tolist()
                    )
                        ...
                        if value_type == "exp_hist":
                            ...
                            for bin_edge, count in map_hist(
                                ...
                                if bin_edge not in df_dict.keys():
                                    ...
                                    # (i.e. -8 to +8 & -16 to +16)
                                    # Need to check if there are other bin \
                                    # edges in the dict and if there \
                                    # length > 1, prefill with zeros
                                    df_dict[bin_edge] = [
                                        *check_for_prefill(df_dict),
                                        count,
                                    ]
                                else:
                            ...
                            max_len, min_len = np.max(hist_list_lens), np.min(
                                hist_list_lens
                            )
                            assert (
                                max_len == min_len
                            ), f"Something has gone wrong with zero \
                                prefills, {max_len},{min_len}"
                            # need to backfill here also (i.e. after all the \
                            # hist counts for the stashs have been added, are \
                            # any lists shorter)
                            ...
                            warnings.warn(
                                "Full tensors cannot be added to a log frame, \
                                    therefore it is being skipped"
                            )
    ...
    df[("metadata", "dtype")] = df[("metadata", "dtype")].apply(
        lambda x: x.split(".")[-1]
    )
    return df
...
def summarise_logframe(df: pd.DataFrame) -> Dict[int, Any]:
    """
    Takes the logframe from step n to step n+m and gets summary stats across \
        each tensor type for each step and returns them in a format suitable \
            for wandb visualisation.

    Args:
        df (pd.DataFrame) the logframe for step n to n+m (m being the offload \
            increment)

    returns:
        Dict[int,Any] a dict of dicts, contains the summary stats for each \
            step.
...
def combine_incremental_dfs(name: str) -> str:
    """
    Combines all the intermediate logframes \
        into a single global one and writes it to disk
    """
    p = Path(f"./{_libname}/{name}/lf")
    files = os.listdir(p)
    # ensure only pickle files
