...
from ... import _config
from ._utils import _dataframe_migration, _validate_schema
...
def read_pickle(
    ...
    """
        Wrapper function for pd.read_pickle. Can only be\
            used to read DataFrames from pickle files \
        unlike the pd version. Asserts the DF to the \
            LogFrame schema and attaches metadata to the DataFrame \
        which is required by downstream functions.

        Args:
        filepath_or_buffer (pd.FilePath | ReadPickleBuffer | \
        WandbArtefactFullName): String, path object (implementing \
        os.PathLike[str]) , or file-like object implementing a \
        binary readlines() \
        function. Also accepts URL. URL is not limited to S3 and GCS
        from_wandb (bool): Retrieve the logframe artifact(s) from wandb \
            (filepath_or_buffer is where you put the WandB artifact fullname)
    ...
    if from_wandb:
        ...
        if not _config._WANDB_EXTRA:
            raise ImportError(
                f"if `from_wandb` == True, wandb must be installed, \
                    please install via `pip install wandb` or \
                        `pip install {_config._libname}[wandb]"
            )
        ...
        df = _pd.read_pickle(filepath_or_buffer, compression, storage_options)
...
def read_parquet(
    ...
    **kwargs,
    ...
    """
        Wrapper function for pd.read_parquet. Can only be used \
            to read DataFrames from parquet files \
        unlike the pd version. Asserts the DF to the \
            LogFrame schema and attaches metadata to the DataFrame \
        which is required by downstream functions.
        ...
        path : str, path object or file-like object
            ...
            gs, and file. For file URLs, a host is expected. \
            ...
            ``file://localhost/path/to/tables`` or \
        ...
        engine : {{'auto', 'pyarrow', 'fastparquet'}}, default 'auto'
            ...
            When using the ``'pyarrow'`` engine and \
                no storage options are provided
            and a filesystem is implemented by \
                both ``pyarrow.fs`` and ``fsspec``
            (e.g. "s3://"), then the ``pyarrow.fs`` \
                filesystem is attempted first.
        ...
        use_nullable_dtypes : bool, default False
            ...
            Note: this is an experimental option, and behaviour \
                (e.g. additional
        ...
        filters : List[Tuple] or List[List[Tuple]], default None
            ...
            Using this argument will NOT result in row-wise filtering \
                of the final
            partitions unless ``engine="pyarrow"`` is also specified.  For
            other engines, filtering is only performed at the partition \
                level, that is,
...
def read_csv(
    ...
    **kwargs,
    ...
    # read DF as per usual.
    df = _pd.read_csv(filepath_or_buffer, *args, **kwargs)
