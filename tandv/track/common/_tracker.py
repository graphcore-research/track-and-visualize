...
from ._log_handler import (
    combine_incremental_dfs,
    global_stash_to_logframe,
    nuke_intermediate_logframes,
    summarise_logframe,
)
from ._types import Event, Stash, StashFn
...
class BaseTracker:
    def __init__(
        ...
        self._name = name if name is not None else randomname.get_name()
        ...
        if self.use_wandb:
            if not _config._WANDB_EXTRA:
                raise ImportError(
                    f"if `use_wandb` == True, wandb must be installed, \
            ...
            self._artifact = wandb.Artifact(
                name=self._name, type=f"{_libname}-logframe"
            )

            if not self.wandb_run:
                raise Exception(
                    "No wandb run has been initialised and you have \
                        selected to use_wandb == True. \
                            Please initialise wandb run."
                )

        # Redundant not that I'm using concurrent.futures

        if async_offload:
            # spawn is preferable here as it eliminates \
            # race condition on _global_stash
    ...
    def stash_event(self, event: Event):
        self.stashes.append(self._stash(event))
    ...
    def offload_global_stash(self, final: bool = False):
        ...
        # converted to DF/Dict and offloaded to disk or sent to wandb
    ...
    def _internal_step(self, *args):
        ...
        # need to clear stashes at the end of every iteration \
        # (to keep torch compile happy as the hooks depend on it)
    ...
    def step(self):
        raise NotImplementedError(
            "This must be implemented for each framework (i.e. capture model \
