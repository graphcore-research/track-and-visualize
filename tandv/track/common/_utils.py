...
def _dataframe_migration(df: pd.DataFrame, schema_map: Any) -> pd.DataFrame:
    """
    Takes an arbitrary dataframe of logs and a schema-map and \
        returns a DataFrame where values are stored under the \
...
def _get_df_schema_hash(df: pd.DataFrame):
    """Gets A SHA-256 Hash of the dataframe \
        'schema' - does not conside attr values"""
...
def _validate_df_hash(df: pd.DataFrame):
    if "vis-meta" not in df.attrs.keys():
        raise SchemaException(
            "The DataFrame schema has not been validated, please load the \
                DataFrame using one of built-in read functions in the library,\
                      or call the validate fn on the existign DF"
        )
    if not df.attrs["vis-meta"]["hash"] == _get_df_schema_hash(df):
        raise SchemaException(
            "The configuration of the DataFrame structure has changed since \
                the file was loaded, please re-validate the DF before \
                    passing it into the vis function"
        )
...
def _validate_schema(df: pd.DataFrame, SCHEMA=_types.LogFrame, debug=False):
    ...
    def _check_wildcard(col):
        # checks if the column tuple matches a wildcard, \
        # if the count is correct,
        # create temporary key mappings -> this assumes that \
        # there's only 1 wild card per higherlevel index \
    ...
    if counter != len(df.columns):
        raise SchemaException(f"The following columns did not \
            match the schema: \
        {','.join([str(umc[1]) for umc in un_matched_cols])}")

    # Make sure WildCards match
    for key, value in wcs.items():
        if value.count > key[1].max or value.count < key[1].min:
            raise SchemaException(
                f"{key[0]}, wild card column count: {value.count}, \
                    is not in required range ({key[1].min} - {key[1].max})"
            )

    if len(matched_keys) != len(schema.keys()):
        bad_cols = ','.join([
            str(key) for key in list(schema.keys()) if
            key not in matched_keys])
        raise SchemaException(f"Missing Required Columns:{bad_cols}")
