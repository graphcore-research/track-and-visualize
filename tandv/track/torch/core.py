...
from ..common._types import Event, Stash, StashFn
...
def track(
    ...
    By default it tracks the stastics for  what we refer to as the \
    ...
    gradients, weights and optimiser state. At the end of training it will \
        write all the logs to a LogFrame (a `pd.DataFrame` which conforms to \
            the schema outline in our docs)
    ...
    Args:
        module (nn.Module): The top level module you wish to track, this \
        would \
            typically be the root class your use to define your model, \
                the tracker will then recursively find all the submodules \
                    and add tracking hooks to their respective tensors
        track_gradients (bool): Whether or not you wish to track the gradients.
        optimizer (torch.optim.Optimizer | None): If you wish to track the \
            optimiser state, pass it as an argument.
        track_weights (bool): Whether or not to track the models \
            weights/parameters.
        ...
        async_offload (bool): If true the set of stashes since last offloaded \
            are serialised and passed to a seperate python process to be \
                converted to a Logframe (currently a very limited \
                    reduction in overhead, but working on improving it)
              ...
              to disk, i.e. offload more frequently to minimise \
                ...
                memory usage. If using wandb, this value should be the \
                    same (or a multiple) of the increment being used to \
                        call `wandb.log'
        offload_type (Literal['.pkl']): The file format you wish to \
            write the LogFrame(s) to disk as.
        use_wandb (bool): If you wish to push the Logframes as \
            artifacts and get summary numerics statistic in wandb. \
                (`offload_inc` should be the same (or a multiple) of \
                    the increment being used to call `wandb.log')
        only_stash_during_training (bool): Torch Tracker can track \
            statistics for Activations when the model is in eval mode \
                (the default behaviour is for this not to be the case \
                    and only track activations during training)
        init_step (int): The tracker has an internal step property for \
            assigning statistics to the correct iteration, if \
                `init_step == None`, defaults to zero, else \
    ...
    """
    if _config._TORCH_EXTRA:
        # assert not (use_wandb and wandb_kws!= None), 'Must provide wandb_kws\
        #  use_wandb==True to init the wandb run'
        ...
        tracker.register_all(
            module, grad=track_gradients, include=include, exclude=exclude
        )
        if optimizer:
        ...
        raise ImportError(
            f"track requires pytorch to be installed, please install \
                via `pip install torch or \
                    `pip install {_config._libname}[torch]"
        )
