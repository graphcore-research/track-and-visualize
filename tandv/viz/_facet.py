...
from seaborn.relational import (  # type: ignore
    _LinePlotter,
    _ScatterPlotter,
    lineplot,
    scatterplot,
...
class FacetGrid(Grid):
    ...
    def refline(self, *, x=None, y=None, color=".5", linestyle="--", **line_kws):
        ...
        color : :mod:`matplotlib color <matplotlib.colors>`
            Specifies the color of the reference line(s). Pass ``color=None``
            touse ``hue`` mapping.
        linestyle : str
        ...
        line_kws : key, value mappings
            ...
            when ``x`` is not None and \
                :meth:`matplotlib.axes.Axes.axhline` when ``y``
            is not None.
    ...
    # and documented by Sphinx

    @property
    def axes(self):
        """An array of the :class:`matplotlib.axes.Axes` \
            objects in the grid."""
        return self._axes

    @property
    def ax(self):
        """The :class:`matplotlib.axes.Axes` when no \
            faceting variables are assigned."""
        if self.axes.shape == (1, 1):
            return self.axes[0, 0]
        else:
            err = "Use the `.axes` attribute when \
                facet variables are assigned."
            raise AttributeError(err)

    @property
    def axes_dict(self):
        """A mapping of facet names to \
            corresponding :class:`matplotlib.axes.Axes`.
...
# Ability to pass in a pre-existing figure rather than generate a new one,
# also calls our version of facegrid rather than seaborns
...
def relplot(
    ...
    **kwargs,
    ...
    if legend:
        ...
        # Ideally, we will have a better solution where \
        # we don't need to re-make
        # the legend out here and will have parity with the \
