...
def _exp_hist_redraw(
    ...
    **kwargs,
    ...
    """
        Essesntially the same as `tandv.viz.exp_hist` except it doesn't \
            generate a new figure, which is the required behaviour for \
                interactive plots.

        Could save some redundant code here by refactoring \
            `tandv.viz.exp_hist` \
            into a public and private fn, and reuse the private fn here? \
    ...
    if not _df.empty:
        if plotter.facet:
            with plt.ioff():
                plotter._plot_facet(df=_df, figure=fig)
        else:
            # create new axes
            new_ax = fig.gca()
            with plt.ioff():
...
def _global_scalar_redraw(
    ...
    **kwargs,
    ...
    """
    Essesntially the same as z `tandv.viz.global_scalar_heatmap` except it \
        doesn't generate a new figure, which is the required behaviour for \
            interactive plots.
    """
    ...
    if not _df.empty:
        # create new axes
        plotter._plot_single(df=_df, ax=new_ax)
...
def _scalar_line_redraw(
    ...
    **kwargs,
    ...
    """
    Essesntially the same as z `tandv.viz.scalar_line` \
        except it doesn't generate a new figure, \
            which is the required behaviour for interactive plots.
    """
    # clear figure
    fig.clear()
    ch_callback = kwargs.pop("ch_callback", None)
    plotter = _ScalarLinePlotter(
        kind=kwargs.pop("kind", _ScalarLineToolbar.kind[0]),
        x=kwargs.pop("x", ("metadata", "step")),
        scalar_metric=scalar_metric,
    ...
    _df = plotter._query(df=df, layer=layer, tt=tt)
...
def interactive(
    f: Callable,
    train_stats: Union[Dict, TrainingStats, None] = None,
    width: int = 1500,
    mouse_sensitivity: int = 10000,
    **kwargs,
) -> None:
    """
        This function makes the visualisation interactive \
    ...
    if not isinstance(train_stats, NoneType) and not TSTATS_ENABLED:
        warnings.warn(
            f"Cross Referencing with Training Stats is not implemented for \
                {f.__name__} and therefore providng train_stats will\
                      do nothing."
        )
    ...
    # initial state of plot (These are set here, so if you need them \
    # the plot can be returned to the init state)
    ...
    def tstats_onpress(
        ...
        """
            ...
            Currently just calls step, and passes the result in as the \
                arguement for the _redraw method in the `WidgetHolder` \
                    object, which overides the value in the \
                        widget with that value.

            Args:
                event (matplotlib.backend_bases.MouseEvent): \
                    The matplotlib mouse press event
                step (Callable): The getter for the step property of the \
                    SnappingCrossHair object (i.e. what training step \
                        is the cross hair at),\
                for use in querying the numerics logs.
                button_press (bool): Not all visualisations \
                    (i.e. scalar_line) need to support \
                        querying numerics data from the training stats \
                this is simply a flag to make the event a no-op \
                    (need to listen for the event either way as it seems \
                        that if button press isn't listened for \
    ...
    def set_up_crosshair(training_ax, other_ax, lines, button_press: bool = False):
        ...
        This closure, initialises a new SnappingCrossHair state \
            management object (in the outer function scope) and \
                disconnecting any old event listeners, etc.

        Args:
            training_ax (...): The matplotlib axis for the training \
                statistics figure (currenly only supports single axis figures)
            other_ax (...): The (axis or axes) of the numerics figure, only \
                used currently in scalar_line which overlays the cross hair \
                    on the numerics axes.
            lines (...): The line(s) in the training axis, used to snap to \
                samples and displaying the info text.
            button_press (...): Where or not the 'button_press_event' \
                handler is a no-op or not.
        ...
        CROSSHAIR = SnappingCrossHair(
            training_ax, other_ax, lines, sensitivity=mouse_sensitivity
        )

        ch_bpe_id = training_fig.canvas.mpl_connect(
            "button_press_event",
            partial(
                tstats_onpress,
                step=CROSSHAIR.get_step_in_crosshairs,
                button_press=button_press,
            ),
        )
        ch_mne_id = training_fig.canvas.mpl_connect(
            "motion_notify_event", CROSSHAIR.on_mouse_move
        )
    ...
    def sgh_onclick(event: matplotlib.backend_bases.MouseEvent):  # type: ignore
        """
            The event handler for the scalar global heatmap \
                'button_press_event'. Facilitates the interaction \
                    of clicking on a patch on the heatmap on \
            navigating to the exponent histogram of that plot and conversely \
                navigating back from the exponent histogram plot back to \
                    heatmap.

            Args:
                event (matplotlib.backend_bases.MouseEvent): The matplotlib \
                    mouse press event

            Returns:
                None
        """
        assert type(WH) is WidgetHolder, "No Widget Holder initalised"
        nonlocal STACK_NUM  # for tracking plot state
        # nonlocal APP

        # where is the mouse w.r.t to x-tickets / y-ticks \
        ...
        if event.button == 1 and STACK_NUM == 0:
            ...
            if tf:
                ...
                # this breaks if not all values are displayed \
                # on the x or y-axis ...
                x_vals = [
                    int(label.get_text())
                    for label in event.canvas.figure.axes[0].get_xticklabels()
                ]
                y_vals = [
                    label.get_text()
                    for label in event.canvas.figure.axes[0].get_yticklabels()
                ]
                prod_xy = list(itertools.product(y_vals, x_vals))
                # use the index of the object in the mesh grid to \
                # get the query params (l_name, Step)
                layer, step = prod_xy[ind["ind"].item()]
                # clear all axes from the figure

                drargs = dict(
                    fig=event.canvas.figure, df=DF, ayer=layer, step=step, **WH.state()
                )
                ...
                WH.rebuild(**get_toolbar(kind=_ExponentHistogramToolbar.kind))
    ...
    # Generate Training Stats Figure (Not implemented for Scalar \
    # Global Heatmap as of yet)
    if not isinstance(train_stats, NoneType) and TSTATS_ENABLED:
        ...
        assert (
            tsteps is not None
        ), "Steps must be provided to use Training stats visualisation"
        tdf = kwargs.get("df", None)
        assert not isinstance(tdf, NoneType), "Must provide a DataFrame"
        assert set(tsteps) == (
            set(tdf.metadata.step.unique())
        ), "The number of steps in train_stats does not \
            match the numerics logging"
        lines = dict()
        with plt.ioff():
    ...
    if f.__name__ == "scalar_global_heatmap":
        _ = fig.figure.canvas.mpl_connect("button_press_event", sgh_onclick)
    ...
    # EXPONENT HISTOGRAM ######
        ...
        _ = fig.figure.canvas.mpl_connect("button_press_event", eh_onclick)
    ...
    # SCALAR GLOBAL LINE ######
        ...
        _ = fig.figure.canvas.mpl_connect("button_press_event", sl_onclick)
        ...
        if not isinstance(train_stats, NoneType) and TSTATS_ENABLED:
            ...
            WH.set_current_redraw_function(
                ...
                fig=fig.figure,
                ch_callback=partial(
                    set_up_crosshair, training_ax=training_ax, lines=lines
                ),
                **kwargs,
            )
        else:
            WH.set_current_redraw_function(
                _scalar_line_redraw, fig=fig.figure, **kwargs
            )
        WH.display()
    ...
    if not isinstance(train_stats, NoneType) and TSTATS_ENABLED:
        APP.append_display_data(
            widgets.HBox(children=[training_fig.canvas, fig.canvas])
        )
    else:
