...
class WidgetHolder:
    """
        The WidgetHolder manages the state of the \
            interactive visualisations toolbar.

        Args
            parent (widgets.Output): The Output widget which \
                the widget holder belongs too.
            **kwargs (widgets): The keyword args are the set of \
                widgets that are manager by the WidgetHolder, with \
                    keyword being the parameter the widget is reponsible for.\
            for example step = widget(), the value from that widget will be \
                used to query w.r.t. the step column in the LogFrame.
    """

    def __init__(self, parent: widgets.Output, **kwargs) -> None:
        self.parent = parent
        self.widgets: Dict[str, widgets.Widget] = {**kwargs}
        ...
        self.container = widgets.HBox(
            children=list(self.widgets.values()), layout=self.hbox_layout
        )
    ...
    def _redraw(self, **kwargs):
        ...
        for k, v in self.widgets.items():
            if (type(v.value) is list or type(v.value) is tuple) and \
                ...
                if type(v.value) is tuple:  # type: ignore
