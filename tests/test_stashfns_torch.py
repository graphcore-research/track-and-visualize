...
from tandv.track.torch import stash_full_tensor, stash_hist, stash_scalar_stats
...
def test_stash_hist_cpu():
    t1, t0, tn = (
        torch.ones((B, N, M), dtype=torch.float32),
        torch.zeros((B, N, M), dtype=torch.float32),
        torch.randn((B, N, M), dtype=torch.float32),
    )
...
def test_stash_hist_cuda():
    t1, t0, tn = (
        ...
        torch.zeros((B, N, M), dtype=torch.float32).to("cuda"),
        torch.randn((B, N, M), dtype=torch.float32).to("cuda"),
    )
